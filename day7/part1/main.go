...
func solve(bags map[string]bag) int {
	count := countContainingBags("shiny gold bag", bags)

	// The golden bag does not count of course.
	return count - 1
}

func countContainingBags(bagName string, bags map[string]bag) int {
	// Each bag "contains" itself
	containedSum := 1

	for containedName, containedCount := range bags[bagName].contains {
		amount := countContainingBags(containedName, bags) * containedCount
		containedSum = containedSum + amount
	}
